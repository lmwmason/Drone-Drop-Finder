...
static void osdElementDropPrediction(osdElementParms_t *element)
{
    DroneDrop_Prediction prediction;

    if (!droneDropFinderGetPrediction(&prediction)) {
        element->buff[0] = '\0';
        return;
    }

    tfp_sprintf(element->buff, "DRP %d,%dM R%dM",
                (int)lrintf(prediction.mean_x_m),
                (int)lrintf(prediction.mean_y_m),
                (int)lrintf(prediction.p95_distance_m));
    // CRITICAL, not WARNING: this only renders once the aircraft has actually
    // entered a failure phase (landing/GPS rescue, or RX loss when
    // triggerOnRxLoss is set) — by the time it's on screen, the pilot needs
    // to notice it.
    element->attr = DISPLAYPORT_SEVERITY_CRITICAL;
    SET_BLINK(element->item);
}

// Physical inputs (altitude/h-speed/v-speed/heading) a search-assistant web
// app needs to look up the matching precomputed grid — separate from
// OSD_DROP_PREDICTION's human-readable summary because all 6 numbers the
// web app needs don't fit in one OSD_ELEMENT_BUFFER_LENGTH (32 char) line.
static void osdElementDropPredictionCombo(osdElementParms_t *element)
{
    float altitudeM, hSpeedMps, vSpeedMps;
    int32_t latE7, lonE7;
    uint16_t headingDeciDeg;

    if (!droneDropFinderGetComboSnapshot(&altitudeM, &hSpeedMps, &vSpeedMps) ||
        !droneDropFinderGetGpsSnapshot(&latE7, &lonE7, &headingDeciDeg)) {
        element->buff[0] = '\0';
        return;
    }

    tfp_sprintf(element->buff, "A%d H%d V%d C%d",
                (int)lrintf(altitudeM),
                (int)lrintf(hSpeedMps * 10.0f),
                (int)lrintf(vSpeedMps * 10.0f),
                (int)(headingDeciDeg / 10));
    element->attr = DISPLAYPORT_SEVERITY_WARNING;
}

// Lat/lon at the failure moment, in plain signed decimal degrees so it can
// be typed directly into a search-assistant web app's URL/form.
static void osdElementDropPredictionGps(osdElementParms_t *element)
{
    int32_t latE7, lonE7;
    uint16_t headingDeciDeg;

    if (!droneDropFinderGetGpsSnapshot(&latE7, &lonE7, &headingDeciDeg)) {
        element->buff[0] = '\0';
        return;
    }
    ...
    buff += tfp_sprintf(buff, "%d.%05d", abs(latE7) / GPS_DEGREES_DIVIDER,
                         (abs(latE7) % GPS_DEGREES_DIVIDER) / 100);
    *buff++ = ',';
    if (lonE7 < 0) {
        *buff++ = SYM_HYPHEN;
    }
    tfp_sprintf(buff, "%d.%05d", abs(lonE7) / GPS_DEGREES_DIVIDER,
                (abs(lonE7) % GPS_DEGREES_DIVIDER) / 100);
    element->attr = DISPLAYPORT_SEVERITY_WARNING;
}
#endif
...
#endif
#ifdef USE_DRONE_DROP_FINDER
    [OSD_DROP_PREDICTION]         = osdElementDropPrediction,
    [OSD_DROP_PREDICTION_COMBO]   = osdElementDropPredictionCombo,
    [OSD_DROP_PREDICTION_GPS]     = osdElementDropPredictionGps,
#endif
#ifdef USE_OSD_PROFILES
...
#ifdef USE_DRONE_DROP_FINDER
    osdAddActiveElement(OSD_DROP_PREDICTION);
    osdAddActiveElement(OSD_DROP_PREDICTION_COMBO);
    osdAddActiveElement(OSD_DROP_PREDICTION_GPS);
#endif
}
