/*
 * Vendored from Drone-Drop-Finder/library/drone_drop.c — see the repo root
 * for the offline Monte-Carlo simulator that generates sweep.csv. Do not
 * hand-edit; port fixes upstream first.
 *
 * The only change from upstream is the added posix_compat.h include below,
 * which routes fopen/fgets/fclose through AP_Filesystem on ArduPilot boards.
 * This file must stay a .c (not .cpp) translation unit for that redirection
 * to apply — posix_compat.h only redefines these for non-C++ code.
 */
